...
- 리뷰 id: `"r-{그룹문자}NN"` 형식 — variant 그룹별 a/b/c/d… + 두 자리 번호
  ...
  - `variant` 라벨은 실제로 존재하는 구매 구분 축(SKU 구성 → 색상 → 유상옵션 순)만 골라
    `" / "`로 이어붙이고, 없는 축은 생략 (예: 색상만 있으면 `"차콜"`, SKU 구성만 다르면
    `"베이직 (거실+방2개)"`, 색상+유상옵션이 모두 있으면 `"단품 / 베이지 / M"`).
    3칸을 고정 템플릿으로 강제하지 않음 — 상품마다 실제 존재하는 축만큼만 채움
- `images` 경로: `/images/reviews/{productId}/review-{NN}-{seq}.webp`
  - `{NN}`은 해당 리뷰 id의 그룹문자를 뗀 두 자리 번호 (예: `r-a07` → `review-07-1.webp`)
  - 포토 리뷰는 전체 기간에 걸쳐 분산 배치하되, **사진이 2~3장인 리뷰는 되도록 배열 앞쪽(최신 쪽)에 배치**할 것.
    단, 가장 최신 리뷰 한 건에만 무조건 사진을 2장씩 몰아주고 나머지는 전부 1장으로 채우는 식은 지양 —
...
## 상세페이지 섹션 카탈로그
- **새 패밀리 생성 시 기본 틀**: sections.ts는 아래 5개 섹션을 `basic → function → material → size → warranty` 순서로 작성해 시작한다:
  | id | label | 내용 |
  |----|-------|------|
...
- 카테고리 고유 섹션이 필요하면 위 5개 외에 새 id를 자유롭게 추가할 수 있음 — `ProductDetailTabs`는 `sections` 배열을 그대로 순회해 탭을 렌더링하므로 별도 등록 절차 없이 바로 사용 가능
- `blocks` 배열 순서: 새로 작성할 때는 하나의 소주제당 `type: "text"`를 먼저, 그 다음에 `type: "image"`를 배치하는 순서로 시작한다 (설명 먼저, 그 설명을 뒷받침하는 이미지가 뒤따르는 순서. 소주제가 여러 개면 이 text→image 쌍을 반복). 강제 규칙은 아니라 필요하면 순서를 바꿔도 됨
- `warranty` 자리는 상품 특성에 따라 `notice`(주의사항) 등 다른 섹션으로 대체되거나, 섹션 순서 자체가 바뀔 수 있음. 다만 새 패밀리를 처음 만들 때는 위 5종 순서를 기본 틀로 삼는다
